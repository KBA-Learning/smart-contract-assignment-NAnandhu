...
contract StudentManagement{
    ...
    modifier checkStudent(uint32 rollno){
        require(studentDetails[rollno].exist,"Roll Number does not exist");
        _;
    }

    function setStudent(string memory _name,uint8 _sem,Department _dept,uint8 _CGPA,uint32 _rollno) public  admin {
    ...
    function editData(uint8 _sem,Department _dept,uint8 _CGPA,uint32 _rollno) public admin checkStudent(_rollno){
    ...
    function editstudentName(string memory _name,uint32 _rollno) public checkStudent(_rollno){
